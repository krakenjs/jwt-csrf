...
var JWT = {
    ...
    pack: function(token, options) {
        ...
        var encryptedToken = {
            token: encrypt(options.secret, JSON.stringify(token), options.algorithm, options.iv)
        };
    ...
    unpack: function(token, options) {
        ...
        return JSON.parse(decrypt(options.secret, encryptedPayload.token,options.algorithm, options.iv));
    }
};
